...
pbounds = {
           'exp_2_tau1': (0, 10), 'exp_2_tau2': (0, 10),
           }
...
def set_model(args, args_due,train_com_loader,num_com):
    ...
    n_inducing_points = args_due.n_inducing_points*30
...
def calculatePerformance(df, file_name):
    ...
    score =  1 / (3 / ( 1/np.exp(float(df['overkill_rate'].tail(1))) + args_due.leakage_weight/np.exp(float(df['leakage_rate'].tail(1))) + 1/np.exp(float(df['unknown_rate'].tail(1)))))
...
def search(exp_2_tau1, exp_2_tau2):
    ...
    for idx, component_name in enumerate(range(num_classes)):
        ...
        if args_due.test_uncertainty == True:
            ...
            uncertainty, labels_train, _, name_list_train, _, _ = get_uncertainty(model, likelihood, train_val_com_loader, tsne=True)
    ...
    if args_due.test_inference == True:
        ...
        name_label_list, y_pred_expert2  = EXP2(args_due ,model ,train_val_loader, test_component_name_df_Expert2, train_component_label, val_component_label, test_component_label ,likelihood ,TH,TH_2,good_num_com)
...
bayes_optimizer.maximize(init_points=50, n_iter=50, acq="ei", xi=0)
